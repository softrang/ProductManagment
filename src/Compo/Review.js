...
const Review = () => {
  ...
  useEffect(()=>{
    ...
    const Mainreview = document.querySelector(".mainreview");
    Mainreview.innerHTML =  "("+Review.length +")";  // Corrected to innerHTML, and removed 'style'
  ...
  return (
    ...
    <p className="text-center text-xl bg-gradient-to-r from-green-700 to-amber-600 text-white font-semibold py-3 rounded-t-lg shadow-inner">Reveiw <span className='mainreview'> </span>  </p>
    
    <div className="w-full  review mt-4 h-max p-3 bg-white rounded-lg shadow-md transform hover:scale-105 transition duration-300 ease-in-out ">
    ...
    {deta.length > 0 ? (
      deta.map((item, index) => (
        item.status === "Review" ? (
          <div key={index} className="order-item review mt-4 bg-white p-4 rounded-lg shadow-lg transform hover:scale-105 transition duration-300 ease-in-out">
