...
const Allorder = () => {
  ...
  useEffect(()=>{
    ...
    Mainall.innerHTML =  "("+All.length +")";  // Corrected to innerHTML, and removed 'style'
  ...
  return (
    ...
    <p className="text-center text-xl bg-gradient-to-r from-blue-700 to-blue-500 text-white font-semibold py-3 rounded-t-lg shadow-inner">All Order <span className='mainall'> </span>   </p>
    
    <div className="w-full all mt-4 h-max p-3 bg-white rounded-lg shadow-md transform hover:scale-105 transition duration-300 ease-in-out ">
    ...
    {deta.length > 0 ? (
      deta.map((item, index) => (
        item.status === "All Order" ? (
          <div key={index} className="order-item all mt-4 bg-white p-4 rounded-lg shadow-lg transform hover:scale-105 transition duration-300 ease-in-out">
