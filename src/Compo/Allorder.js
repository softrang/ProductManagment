...
const AllorderItem = ({ item, index, onDelete }) => (
...
const Allorder = ({searchQuery}) => {
  const dispatch = useDispatch();
  const deta = useSelector((state) => state.product.deta);
  const [allorderCount, setallorderCount] = useState(0);

  useEffect(() => {
    const count = deta ? deta.filter(item => item.status === "All Order").length : 0;
    setallorderCount(count);
  }, [deta]);
...
const data = deta.filter(item => item.status === "All Order").slice(-2).reverse()
  ...
  return (
    <div className="w-full h-max  bg-gradient-to-br from-gray-800 to-gray-800 p-4 rounded-lg shadow-md shadow-gray-800 ">
      <p className="text-center text-xl bg-gradient-to-r from-sky-700 to-sky-600 text-white font-semibold py-3 rounded-t-lg shadow-inner">
        All Order <span>({allorderCount})</span>
      </p>
      ...
      {data && data.length > 0 ? (
        data.filter((item) => 
          ...
          .map((item, index) => (
            <AllorderItem key={index} item={item} index={index} onDelete={onDelete} />
          ))
      ) : (
