...
const Complete = () => {
  ...
  useEffect(()=>{
    ...
    const Maincom = document.querySelector(".maincom");
    Maincom.innerHTML =  "("+Com.length +")";  // Corrected to innerHTML, and removed 'style'
  ...
  return (
    ...
    <p className="text-center text-xl bg-gradient-to-r from-green-700 to-green-500 text-white font-semibold py-3 rounded-t-lg shadow-inner">Complete <span className='maincom'> </span> </p>
    
    <div className="w-full mt-4 com h-max p-3 bg-white rounded-lg shadow-md transform hover:scale-105 transition duration-300 ease-in-out ">
    ...
    {deta.length > 0 ? (
      deta.map((item, index) => (
        item.status === "Complete" ? (
          <div key={index} className="order-item com mt-4 bg-white p-4 rounded-lg shadow-lg transform hover:scale-105 transition duration-300 ease-in-out">
